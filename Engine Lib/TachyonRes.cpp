...
void CTachyonRes::LoadTEX( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%u_%sS.IDX", m_pDEVICE->m_option.m_nTextureDetail, strGroupID);
...
void CTachyonRes::LoadIMGBUF( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sList.LST", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
...
void CTachyonRes::LoadIMG( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sI.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
	...
	{
		MessageBox(0, _T("Error in Indexes"), _T("Something Happened"), 0);
	}
}
...
void CTachyonRes::LoadMEDIA( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sW.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
...
void CTachyonRes::LoadANI( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sA.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
...
void CTachyonRes::LoadMESH( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sM.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
...
void CTachyonRes::LoadOBJ( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sO.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
...
void CTachyonRes::LoadSFX( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sX.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
...
void CTachyonRes::LoadMAP( CString strGroupID)
{
	...
	strINDEX.Format( ".\\Data\\Index\\%sP.IDX", strGroupID);
	CFile file( strINDEX, CFile::modeRead|CFile::typeBinary);
							  ...
							  int nTotal)
{
	...
	while( dwPOS < dwLENGTH )
	{
		...
		positionEnabled.Format("Img\\%u.png",dwID);
		...
		pTEX->LoadT3DTEX(
			...
			dwSIZE,
			bFormat);
//		pTEX->SaveImageFile(positionEnabled,D3DXIFF_PNG,pTEX->GetTEX());
						   ...
						   int nTotal)
{
	...
	CString pos;
	CString file;
	file.Format("img\\%u.png",pIMG->GetImage());

pIMGSRC->SaveImageFile(m_pDEVICE->m_pDevice,file,D3DXIFF_PNG);
		...
		for( i=0; i<nCount; i++)
		{
			...
			file.Read( &pKEY->m_dwTick, sizeof(DWORD));
			file.Read( &pKEY->m_dwColor, sizeof(DWORD));

			pIMG->m_vKey.push_back(pKEY);
		}

		file.Read( &pIMG->m_dwTotalTick, sizeof(DWORD));
...
/*		catch (int chyba = 1)
		{
			MessageBox(0, _T("ChyT happened again, Error on IMG, so it wasn't maybe at IDX Stupid konst how much I hate him"), _T("Y"), 0);
		}
	}
	*/
}
...
//		CString pos;
//		pos.Format("Img\%u.png",dwPOS);
//		pMESH->SaveImageFile(pos,D3DXIFF_PNG,);
